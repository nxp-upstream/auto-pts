...
def test_cases(ptses):
    ...
    for tc_name in test_case_name_list:        
        if tc_name.startswith("AVDTP/SRC/"):
            instance = ZTestCase("AVDTP", tc_name,
                                cmds=pre_conditions +
                                [TestFunc(btp.a2dp_register_ep, defs.BTP_A2DP_CMD_ROLE_SOURCE, defs.BTP_A2DP_CMD_CODEC_SBC)],
                                generic_wid_hdl=avdtp_wid_hdl)
        elif tc_name.startswith("AVDTP/SNK/"):
            instance = ZTestCase("AVDTP", tc_name,
                                cmds=pre_conditions +
