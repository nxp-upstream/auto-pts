...
def hdl_wid_19(_: WIDParams):
    """
    description: Please wait while the tester verifies that the IUT does not respond to the invalid DISCOVER command sent by the tester.
    """
    return True
...
def hdl_wid_22(_: WIDParams):
    """
    description: Take action to reject the GET CONFIGURATION sent by the tester.  The IUT is expected to respond with BAD_ACP_SEID because the SEID requested was not previously configured.
    """
    return True
...
def hdl_wid_26(_: WIDParams):
    """
    description: Take action to reject the SET CONFIGURATION sent by the tester.  The IUT is expected to respond with SEP_IN_USE because the SEP requested was previously configured.
    """
    return True
